...
function parseDateOptional(dateStr) {
  ...
  try {
    ...
    throw new Error(
      "无效的日期格式。请使用 YYYY-MM-DD 或 YYYY-MM-DD HH:MM:SS。",
    );
  }
}
...
function getTimeRangeFromPeriod(timePeriodStr, referenceDateStr = null) {
  ...
  if (timePeriodLower === "today" || timePeriodLower === "今天") {
    ...
    endOfLastWeek.setDate(
      today.getDate() - today.getDay() - (today.getDay() === 0 ? 0 : 0),
    );
...
class ApiService {
  ...
  async getCategoryDistribution(options = {}) {
    ...
    const transactions = await this.getFilteredTransactionList({
      ...
      timePeriod,
      incomeExpenseType:
        incomeExpenseFocus === "all" ? null : incomeExpenseFocus,
  ...
  async getTimeSeries(options = {}) {
    ...
    if (!queryStartDate || !queryEndDate) {
      throw new Error(
        `时间序列数据的日期范围无效: ${startDateStr} 到 ${endDateStr}`,
      );
    }
  ...
  async getChartData(filters = {}) {
    try {
      ...
      const combinedCategories =
        allFilteredCategories.length > 0 ? allFilteredCategories : null;
      ...
      const expenseCategoryDistribution = await this.getCategoryDistribution({
        ...
        parentCategories:
          expenseCategories.length > 0 ? expenseCategories : null,
      });
