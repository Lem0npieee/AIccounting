        ...
        <div class="category-list">
          ...
          >
            <div
              ...
              :style="{
                backgroundColor:
                  incomeChartColors[index % incomeChartColors.length],
              }"
            ></div>
            <div class="category-name">{{ item.label }}</div>
            <div class="category-value">¥{{ item.value.toFixed(2) }}</div>
...
const incomeCategories = ["工资", "奖金", "补贴", "兼职", "投资", "其他收入"];
...
const fetchFilteredData = async () => {
  ...
  try {
    ...
    const chartFilters = {
      ...
      timeUnit: filterTimeType.value,
      expenseCategories:
        filterExpense.value.length > 0 ? filterExpense.value : [],
    ...
    // 处理饼图数据
    expenseChartData.value = chartData.expenseCategoryDistribution
      .map((item) => ({
        label: item.category,
        value: item.amount,
      }))
    ...
    incomeChartData.value = chartData.incomeCategoryDistribution
      .map((item) => ({
        label: item.category,
        value: item.amount,
      }))
